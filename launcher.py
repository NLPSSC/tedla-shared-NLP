...
PROJECT_ROOT = str(Path(__file__).resolve().parent / "tedla-hypertension-project-databricks")
SRC_DIR = str(Path(PROJECT_ROOT) / "src")

SEARCH_TERMS = [
    "angioedema", "ascvd score", "bradycardia", "breast enlargement",
    "breast pain", "constipation", "dec. libido", "decreased heart rate",
    "decreased libido", "dizziness", "doe", "drowsiness", "dry cough",
    "dry mouth", "dryness of the mouth", "dyspnea on exertion", "ed",
    "elevated hr", "erectile disfunction", "excessive facial hair",
    "excessive sleepiness", "face swelling", "facial erythema", "falling out",
    "fatigue", "flushing", "gingival hypertrophy", "gout", "gouty flare",
    "gum swelling", "gynecomastia", "h/a", "ha", "hallucinations", "headache",
    "heart failure with preserved ejection fraction",
    "heart failure with reduced ejection fraction", "hepatotoxicity", "hfpef",
    "hfref", "high ca", "high calcium", "hirsutism", "hyper ca", "hyper k",
    "hyper ua", "hypercalcemia", "hyperkalemia", "hyperprolactinemia",
    "hyperuricemia", "hypokalemia", "hyponatremia", "impotence", "inc hr",
    "inc. wt.", "increase in exercise", "increased heart rate",
    "increased lfts", "increased liver function tests", "increased prolactin",
    "increased weight", "insomnia", "light headedness", "lip swelling",
    "liver toxicity", "low alcohol intake", "low blood pressure", "low bp",
    "low hr", "low k", "low na", "low salt intake", "low sodium",
    "lower k", "lupus sx", "lupus symptom", "lupus-like syndrome",
    "mastodynia", "medication adherence", "n/v", "nausea", "nightmare",
    "nonproductive cough", "orthostatic hypotension", "parasomnia",
    "parkinsonian symptoms", "parkinsonism", "passed out", "physical activity",
    "poor sleep", "positive ana", "positive hgc", "pregnancy", "pregnant",
    "sexual se", "sexual side effect", "shortness of breath",
    "sick sinus syndrome", "sob", "somnolence", "symptoms of parkinson",
    "syncope", "tachycardia", "tired", "tongue swelling", "ua high",
    "urgency", "uric acid elevated", "urinary urgency", "vivid dreams",
    "vomiting", "weight gain", "weight loss", "weight reduction",
    "hypo k", "low sodium",
]

NOTE_TYPE_NAMES = [
    "outpatient", "inpatient", "admission", "discharge_summary",
    "emergency_department", "problem_list", "ecg_impression",
...
ALL_GROUP_COLS = [
    "problem_list_notes_group", "outpatient_notes_group",
    "communication_encounter_notes_group", "inpatient_notes_group",
    "admission_notes_group", "emergency_department_notes_group",
    "ecg_impression_notes_group", "discharge_summary_notes_group",
]
...
def _embed(term, ctx):
    if ctx == "positive":
        t = [f"Patient reports {term}.", f"Noted {term} on examination.",
             f"Assessment: {term}.", f"Patient presents with {term}."]
    elif ctx == "negated":
        t = [f"Patient denies {term}.", f"No evidence of {term}.",
             f"Negative for {term}.", f"No {term} reported."]
    else:
        t = [f"Family history significant for {term}.",
             f"Mother had {term}.", f"Father diagnosed with {term}."]
...
def _bp(): return f"{random.randint(110,180)}/{random.randint(60,110)}"
def _hr(): return random.randint(55, 110)
...
def _note_problem_list(terms):
    ...
    for t in tl[:3]: probs.append(t.rstrip("."))
    probs.extend(random.sample(["Type 2 diabetes", "Obesity BMI 32", "CKD stage 2", "OSA", "GERD"], k=random.randint(1,3)))
    return "PROBLEM LIST (Active)\n" + "\n".join(f"  {i+1}. {p}" for i, p in enumerate(probs)) + "\n"

def _note_ecg(terms):
    rate = random.randint(50, 120)
    rhythm = random.choice(["Normal sinus rhythm", "Sinus tachycardia", "Sinus bradycardia"])
...
NOTE_GEN = {
    "outpatient": _note_outpatient, "inpatient": _note_inpatient,
    "admission": _note_admission, "discharge_summary": _note_discharge,
    "emergency_department": _note_ed, "problem_list": _note_problem_list,
    "ecg_impression": _note_ecg, "communication_encounter": _note_comm,
}
...
def generate_test_data(output_folder, num_notes, log_fn=print):
    ...
    dirs = {k: inp / k for k in ["note_data","data_to_process","note_to_groups_map","note_to_visit_date","note_group_to_term"]}
    ...
    for i in range(num_notes):
        ...
        meta.append({"note_id":nid,"person_id":pid,"x_part_num":1,"x_uniq":str(900000+i),
                      "note_date":nd,"visit_occurrence_id":vid,
                      "is_patient_communication":nt=="communication_encounter",
                      "note_within_visit":nt in ("inpatient","admission","discharge_summary","emergency_department")})
        gr = {"note_id": nid}
        for c in ALL_GROUP_COLS: gr[c] = (c == NOTE_TYPE_TO_GROUP_COL[nt])
        groups.append(gr)
        visits.append({"note_id":nid,"visit_occurrence_id":vid,"visit_start_date":nd,"visit_end_date":nd+timedelta(days=random.randint(0,3))})
    log_fn("Writing parquet files...")
    df=pd.DataFrame(notes); df["note_id"]=df["note_id"].astype("int64")
    df.to_parquet(dirs["note_data"]/"synthetic_notes.parquet",index=False)
    df=pd.DataFrame(meta); df["note_id"]=df["note_id"].astype("int64"); df["person_id"]=df["person_id"].astype("int64")
    df["x_part_num"]=df["x_part_num"].astype("int32"); df["visit_occurrence_id"]=df["visit_occurrence_id"].astype("int64")
    df["note_date"]=pd.to_datetime(df["note_date"])
    df.to_parquet(dirs["data_to_process"]/"data_to_process.parquet",index=False)
    df=pd.DataFrame(groups); df["note_id"]=df["note_id"].astype("int64")
    for c in ALL_GROUP_COLS: df[c]=df[c].astype(bool)
    df.to_parquet(dirs["note_to_groups_map"]/"note_to_groups_map.parquet",index=False)
    df=pd.DataFrame(visits); df["note_id"]=df["note_id"].astype("int64"); df["visit_occurrence_id"]=df["visit_occurrence_id"].astype("int64")
    df["visit_start_date"]=pd.to_datetime(df["visit_start_date"]); df["visit_end_date"]=pd.to_datetime(df["visit_end_date"])
    df.to_parquet(dirs["note_to_visit_date"]/"note_to_visit_date.parquet",index=False)
    csv_p = Path(PROJECT_ROOT)/"src"/"nlp_method"/"data"/"map_search_term_to_note_group.csv"
    if csv_p.exists():
        pd.read_csv(csv_p).to_parquet(dirs["note_group_to_term"]/"note_group_to_term.parquet",index=False)
    else:
        recs = [{"search_term_to_note_groups_id":i+1,"search_term":t,"notes_admissions":1,
                  "notes_communication_encounter":1 if random.random()>0.3 else 0,"notes_discharge_summary":1,
                  "notes_ecg_impression":1 if t in ["bradycardia","tachycardia"] else 0,
                  "notes_emergency_department":1,"notes_inpatient":1,"notes_outpatient":1,"notes_problem_lists":1}
                 for i,t in enumerate(sorted(set(SEARCH_TERMS)))]
        pd.DataFrame(recs).to_parquet(dirs["note_group_to_term"]/"note_group_to_term.parquet",index=False)
...
def make_env_file(input_path, output_path, num_workers):
    ...
    dp = str(inp/"data_to_process") if (inp/"data_to_process").exists() else str(inp)
    gm = str(inp/"note_to_groups_map") if (inp/"note_to_groups_map").exists() else ""
    vd = str(inp/"note_to_visit_date") if (inp/"note_to_visit_date").exists() else ""
    gt = str(inp/"note_group_to_term") if (inp/"note_group_to_term").exists() else ""
    cs = Path(PROJECT_ROOT)/"src"/"nlp_method"/"data"/"map_search_term_to_note_group.csv"
    for d in (out/"results", out/"logs", out/"metrics"): d.mkdir(parents=True, exist_ok=True)
...
def import_notes_file(file_path, output_folder, log_fn=print):
    ...
    if "note_text" not in df.columns:
        raise ValueError("File must have a 'note_text' column containing the clinical note text.")
    ...
    if "note_date" not in df.columns:
        ...
        df["note_date"] = pd.to_datetime(df["note_date"], errors="coerce").fillna(pd.Timestamp.now().normalize())
    ...
    for c in ALL_GROUP_COLS:
        if note_type_col:
            ...
            groups[c] = mapped_type.apply(lambda v, col=c: col.replace("_notes_group","") in str(v))
        else:
            groups[c] = (c == "outpatient_notes_group")  # default all to outpatient
    groups.to_parquet(dirs["note_to_groups_map"] / "note_to_groups_map.parquet", index=False)
    ...
    visits.to_parquet(dirs["note_to_visit_date"] / "note_to_visit_date.parquet", index=False)
    ...
    if csv_p.exists():
        ...
        recs = [{"search_term_to_note_groups_id": i+1, "search_term": t,
                  "notes_admissions":1, "notes_communication_encounter":1,
                  "notes_discharge_summary":1, "notes_ecg_impression":0,
                  "notes_emergency_department":1, "notes_inpatient":1,
                  "notes_outpatient":1, "notes_problem_lists":1}
                 for i, t in enumerate(sorted(set(SEARCH_TERMS)))]
        pd.DataFrame(recs).to_parquet(dirs["note_group_to_term"] / "note_group_to_term.parquet", index=False)
...
RESULT_COLUMNS = [
    "note_id", "batch_group", "patient_id", "window_text", "note_date",
    "search_term", "problem_list_notes_group", "outpatient_notes_group",
    "communication_encounter_notes_group", "inpatient_notes_group",
    "admission_notes_group", "emergency_department_notes_group",
    "ecg_impression_notes_group", "discharge_summary_notes_group",
    "is_patient_communication", "is_note_within_visit", "is_negated",
    "window_start_char_offset", "window_end_char_offset",
    ...
    def _work():
        try:
            ...
            if req.exists():
                ...
                _runcmd([py,"-m","pip","install","spacy>=3.7","medspacy>=1.3","pandas","pyarrow","polars","python-dotenv","loguru"], log_fn)
            log_fn("[2/2] Downloading spaCy model...")
            _runcmd([py,"-m","spacy","download","en_core_web_sm"], log_fn)
            log_fn("Verifying...")
            _runcmd([py,"-c","import spacy; import medspacy; import pandas; import pyarrow; import polars; import loguru; print('All OK')"], log_fn)
...
def _runcmd(cmd, log_fn):
    ...
    for line in p.stdout: log_fn(line.rstrip("\n"))
    p.wait()
    if p.returncode != 0: raise RuntimeError(f"Command failed (exit {p.returncode})")
...
class PipelineRunner:
    ...
    def count_notes(self, input_path):
        try:
            ...
            d = Path(input_path)
            if (d/"note_data").exists(): d = d/"note_data"
            return sum(pq.read_metadata(str(f)).num_rows for f in d.glob("**/*.parquet"))
        except: return 0

    def start(self):
        self.notes_sent = 0; self._running = True
        env = os.environ.copy(); env["PYTHONPATH"] = SRC_DIR
        try:
        ...
        self._proc = subprocess.Popen([sys.executable,"-m","nlp_method"], cwd=PROJECT_ROOT, env=env,
    ...
    def returncode(self): return self._proc.poll() if self._proc else None
    ...
    def _reader(self):
        try:
            for line in self._proc.stdout:
                ...
                if m: self.notes_sent = int(m.group(1))
                self._q.put((line, any(k in line.lower() for k in ["error","exception","traceback","failed"])))
        except: pass
        finally:
            self._proc.wait(); self._running = False
...
class RoundedButton(tk.Canvas):
    ...
    def __init__(self, parent, text="", command=None, color=None, width=160, height=38, **kw):
        super().__init__(parent, width=width, height=height, bg=C["surface"],
        ...
        self.bind("<Enter>", lambda e: self._draw(self._hover_color) if self._enabled else None)
        self.bind("<Leave>", lambda e: self._draw(self._color) if self._enabled else None)
        self.bind("<Button-1>", lambda e: self._cmd() if self._cmd and self._enabled else None)
    ...
    def _draw(self, fill):
        ...
        self.create_text(w//2, h//2, text=self._text, fill=fg, font=("Segoe UI", 10, "bold"))
...
class App:
    def __init__(self, root):
        ...
        tk.Label(hdr, text="TEDLA", font=("Segoe UI", 22, "bold"),
                 fg=C["accent"], bg=C["bg"]).pack(side="left")
        tk.Label(hdr, text="  Hypertension NLP Pipeline", font=("Segoe UI", 22),
                 fg=C["text"], bg=C["bg"]).pack(side="left")
        tk.Label(hdr, text="Clinical note processing for research",
                 font=("Segoe UI", 10), fg=C["text2"], bg=C["bg"]).pack(side="right", pady=8)

        # ---- Card: Install ----
        self._card_install = self._card(main, "Install Dependencies",
            "Download all required packages and language models")
        ci = self._card_install
        row = tk.Frame(ci, bg=C["surface"])
        row.pack(fill="x", pady=(8, 0))
        self.btn_install = RoundedButton(row, "Install", self._install, width=130, height=36)
        self.btn_install.pack(side="left")
        self.lbl_install = tk.Label(row, text="  Not installed yet", font=("Segoe UI", 9),
                                     fg=C["text2"], bg=C["surface"])
        self.lbl_install.pack(side="left", padx=10)

        # ---- Card: Run Pipeline ----
        self._card_run = self._card(main, "Run Pipeline",
        ...
        tk.Label(r1, text="Input", font=("Segoe UI", 9), fg=C["text2"],
                 bg=C["surface"], width=8, anchor="w").pack(side="left")
        self.inp_var = tk.StringVar()
        tk.Entry(r1, textvariable=self.inp_var, font=("Segoe UI", 10),
                 bg=C["surface2"], fg=C["text"], insertbackground=C["text"],
                 relief="flat", bd=0).pack(side="left", fill="x", expand=True, ipady=6, padx=(0, 6))
        self._browse_btn(r1, self.inp_var)
        self.btn_import = RoundedButton(r1, "Import CSV/Excel", self._import_file,
                                         color="#7c3aed", width=145, height=36)
        self.btn_import.pack(side="right", padx=(6, 0))
        # Output
        r2 = tk.Frame(cp, bg=C["surface"])
        r2.pack(fill="x", pady=(0, 4))
        tk.Label(r2, text="Output", font=("Segoe UI", 9), fg=C["text2"],
                 bg=C["surface"], width=8, anchor="w").pack(side="left")
        self.out_var = tk.StringVar()
        tk.Entry(r2, textvariable=self.out_var, font=("Segoe UI", 10),
                 bg=C["surface2"], fg=C["text"], insertbackground=C["text"],
                 relief="flat", bd=0).pack(side="left", fill="x", expand=True, ipady=6, padx=(0, 6))
        ...
        tk.Label(r3, text="Workers", font=("Segoe UI", 9), fg=C["text2"],
                 bg=C["surface"], width=8, anchor="w").pack(side="left")
        self.wk_var = tk.StringVar(value=str(min(4, os.cpu_count() or 2)))
        tk.Entry(r3, textvariable=self.wk_var, font=("Segoe UI", 10), width=5,
                 bg=C["surface2"], fg=C["text"], insertbackground=C["text"],
                 relief="flat", bd=0).pack(side="left", ipady=6, padx=(0, 6))
        tk.Label(r3, text=f"of {os.cpu_count() or '?'} cores", font=("Segoe UI", 9),
                 fg=C["text2"], bg=C["surface"]).pack(side="left", padx=(0, 16))
        self.btn_run = RoundedButton(r3, "Run Pipeline", self._run, color="#16a34a", width=150, height=36)
        self.btn_run.pack(side="left", padx=(0, 8))
        self.btn_stop = RoundedButton(r3, "Stop", self._stop, color=C["red"], width=80, height=36)
        self.btn_stop.pack(side="left", padx=(0, 8))
        self.btn_stop.set_enabled(False)
        self.btn_export = RoundedButton(r3, "Export CSV", self._export_results, color="#0891b2", width=110, height=36)
        self.btn_export.pack(side="left", padx=(0, 8))
        self.btn_export.set_enabled(False)
        self.btn_open = RoundedButton(r3, "Open Output", self._open_out, color=C["surface2"], width=120, height=36)
        ...
        tk.Label(r4, textvariable=self.prog_var, font=("Segoe UI", 9),
                 fg=C["accent_h"], bg=C["surface"]).pack(side="left")
        # Progress bar canvas
        self.prog_canvas = tk.Canvas(cp, height=6, bg=C["surface2"], highlightthickness=0)
        ...
        log_hdr.pack(fill="x")
        tk.Label(log_hdr, text="  Log Output", font=("Segoe UI", 9, "bold"),
                 fg=C["text2"], bg=C["surface"], pady=4).pack(side="left")
        clr_btn = tk.Label(log_hdr, text=" Clear ", font=("Segoe UI", 8),
                           fg=C["text2"], bg=C["surface2"], cursor="hand2", padx=8, pady=2)
        clr_btn.pack(side="right", padx=6, pady=3)
        clr_btn.bind("<Button-1>", lambda e: self._clear_log())

        self.log = tk.Text(log_frame, wrap="word", font=("Cascadia Code", 9),
                           bg=C["log_bg"], fg=C["log_fg"], insertbackground=C["text"],
                           state="disabled", bd=0, padx=10, pady=6, relief="flat")
        sb = tk.Scrollbar(log_frame, command=self.log.yview, bg=C["surface"],
        ...
        tk.Label(main, textvariable=self.stat_var, font=("Segoe UI", 9),
                 fg=C["text2"], bg=C["bg"], anchor="w", pady=4).pack(fill="x")
    ...
    def _card(self, parent, title, subtitle):
        ...
        tk.Label(inner, text=subtitle, font=("Segoe UI", 9),
                 fg=C["text2"], bg=C["surface"]).pack(anchor="w")
        return inner

    def _browse_btn(self, parent, var):
        b = tk.Label(parent, text=" ... ", font=("Segoe UI", 9, "bold"),
                     fg=C["text"], bg=C["surface2"], cursor="hand2", padx=8, pady=4)
        b.pack(side="right")
        b.bind("<Button-1>", lambda e: var.set(
    ...
    def _set_progress(self, pct):
        ...
        if w > 1:
            ...
            self.prog_canvas.create_rectangle(0, 0, fill_w, 6, fill=C["accent"], outline="")
    ...
    def _install(self):
        ...
        def log_cb(line): self.root.after(0, self._w, line + "\n")
        def done_cb(ok, msg):
            def _u():
    ...
    def _import_file(self):
        ...
        def _work():
            try:
                inp, out = import_notes_file(fp, folder,
    ...
    def _export_results(self):
        ...
        if not out:
            messagebox.showwarning("Missing", "No output folder set."); return
        ...
        fp = filedialog.asksaveasfilename(
            ...
            filetypes=[("CSV file","*.csv"),("Excel file","*.xlsx"),("TSV file","*.tsv")])
        if not fp: return
        self._w("--- Exporting results ---\n", "dim")
        def _work():
            try:
                path, count = export_results(str(results_dir), fp,
                    log_fn=lambda msg: self.root.after(0, self._w, msg + "\n"))
    ...
    def _run(self):
        ...
        out = self.out_var.get().strip()
        if not inp: messagebox.showwarning("Missing", "Select an Input folder."); return
        if not out: messagebox.showwarning("Missing", "Select an Output folder."); return
        if not Path(inp).exists(): messagebox.showerror("Error", f"Input not found:\n{inp}"); return
        try:
            wk = int(self.wk_var.get())
        except ValueError:
            messagebox.showwarning("Invalid", "Workers must be a number."); return
        ...
        if self._total > 0:
            ...
            es = f"~{max(1,int(est*60))}s" if est < 1 else (f"~{int(est)}m" if est < 60 else f"~{est/60:.1f}h")
    ...
    def _poll(self):
        ...
        if self._total > 0 and self.runner:
            ...
            if sent > 0 and elapsed > 0:
                ...
                rs = f"{int(rem)}s left" if rem < 60 else f"{int(rem//60)}m {int(rem%60)}s left"
